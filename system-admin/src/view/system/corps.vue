      ...
      <Row class="margin-top-10">
        <Table :columns="columns" :data="tableData" border stripe highlight-row>
          <template slot-scope="{ row, index }" slot="action">
            ...
            <Button type="error" size="small" @click="deleteConfirm(row.corpId)">删除</Button>
          </template>
        </Table>
    ...
    <div>
      <Drawer :closable="false" width="640" v-model="isShowDetailDrawer">
        ...
        <div class="detail-drawer-profile">
          <Row>企业名称: {{entity.corpName}}</Row>
          <Row>企业Key: {{entity.corpKey}}</Row>
    ...
    <div>
      <Drawer :closable="false" width="640" v-model="isShowSaveDrawer">
        ...
        <Form :model="entity" :label-width="120">
          <div class="detail-drawer-profile">
            <FormItem label="企业名称"><Input v-model="entity.corpName" placeholder="企业名称" /></FormItem>
            <FormItem label="企业Key"><Input v-model="entity.corpKey" placeholder="企业Key" /></FormItem>
            <FormItem label="状态">
...
<script>
  ...
  import { getCorp, getCorps, saveCorp, deleteCorp } from '@/api/corp'
  export default {
    name: 'system-corps',
    data() {
      return {
